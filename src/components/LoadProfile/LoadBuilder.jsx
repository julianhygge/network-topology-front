...
import {
  deleteLoadProfile,
  fetchLoadProfileItems,
  saveLoadProfileItems,
...
const LoadBuilder = () => {
  ...
  const onAdd = ({
    ...
    setLoads([
      ...loads,
      {
        electrical_device_id: parseInt(electricalDeviceId),
        rating_watts: ratingWatts,
        quantity,
        hours,
        total: addedTotal,
        isNew: true,
  ...
  const saveLoads = () => {
    setLoading(true); // Show loader popup
    const save = async () => {
      try {
        ...
        setLoading(false); // Hide loader popup
      }
    };
    save();
  };
  ...
  return (
    <>
      {loading && <LoaderPopup message="Processing, please wait..." />}{" "}
      {/* Loader popup */}
              ...
              <div className="nav pt-4">
                <ul className="pt-1 pb-3">
                  <li className="heading font-light">
                    Please continue to do the configuration from Load Builder
                    below.
                  </li>
                  <li className="watts font-normal">Total = {total} Watts</li>
                  <li
                    className="reset-profile mr-1 cursor-pointer py-1"
                    ...
                    <React.Fragment key={index}>
                      <ul className="py-4 border-b border-gray-300 bg-[#F9FEFF]">
                        <li className="device-type-column">
                          {electricalIdToName(item.electrical_device_id)}
                        </li>
